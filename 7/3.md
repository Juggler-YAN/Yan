...
可以,但在多层神经网络中会出现权重对称性导致隐藏神经元对称性,使得多个隐藏神经元作用等效于一个神经元
...
由普朗克黑体辐射定律可得,$T=\frac{hv}{Kln(1+\frac{8\pi hv^3}{c^3u_v})}$
...
$$
\begin{align}
Var_{softmax(\bold{o})} &= \sum \limits_{j=1}^q{(softmax(\bold{o})_j-E[softmax(\bold{o})_j])^2} \nonumber \\
&= \sum \limits_{j=1}^q {(softmax(\bold{o})_j-\frac{1}{q}\sum \limits_{k=1}^qsoftmax(\bold{o})_k)^2} \nonumber \\
&= \sum \limits_{j=1}^q {(softmax(\bold{o})_j-\frac{1}{q})^2} \nonumber \\
\end{align}
$$
