...
\end{equation}
$$
即单个ReLU函数是连续的分段线性函数，所以多个ReLU函数的组合仍然是连续的分段线性函数
...
$argmaxP(\bold{w}|\bold{x}) \leftrightarrow argmaxP(\bold{x}|\bold{w})P(\bold{w}) \leftrightarrow argmin\sum-\ln{P(x_i|w)-\ln{P(w)}}$，若$\bold{w} \sim N(0,\sigma^2)$，则$P(\bold{w}) \sim \exp(-\frac{\Vert \bold{w} \Vert_2}{2\sigma^3})$，即$P(\bold{w})$和正则化联系为$-\ln{P(\bold{w})} \sim \frac{\Vert \bold{w} \Vert_2}{2\sigma^3}=\lambda \Vert \bold{w} \Vert_2$
