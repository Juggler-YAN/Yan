...
if __name__ == '__main__':
    ...
    while step < num_steps and not num_steps_reached:
        for tokens_X, segments_X, valid_lens_x, pred_positions_X,\
            ...
            mlm_l, nsp_l, l = _get_batch_loss_bert(
                net, loss, len(vocab), tokens_X, segments_X, valid_lens_x,
            ...
            metric[3] += 1
            MLM_loss.append((metric[0] / metric[3]).item())
            NSP_loss.append((metric[1] / metric[3]).item())
            step += 1
            if step == num_steps:
    ...
    # plot
    plt.plot(MLM_loss, label='MLM')
    plt.plot(NSP_loss, label='NSP')
    plt.legend()
    plt.savefig("1.png")
    ...
    encoded_text_crane = encoded_text[:, 1, :]
