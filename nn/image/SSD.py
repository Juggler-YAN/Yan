...
def read_data_bananas(is_train=True):
    """读取香蕉检测数据集中的图像和标签"""
    csv_fname = os.path.join('./data/banana-detection', 'bananas_train' if is_train
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        loss_cls.append(cls_err)
        loss_bbox.append(bbox_mae)
    plt.plot(loss_cls, label='cls')
    plt.plot(loss_bbox, label='bbox')
    plt.legend()
    plt.savefig("1.png")
